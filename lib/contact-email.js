// Shared mailto decision core used by Settings and ExternalLink.
//
// openContactEmail() optionally checks whether the device can handle the mailto
// URL, opens the mail composer, and shows the appropriate fallback alert when
// mail is unavailable or the attempt fails. Android callers skip the capability
// check because package visibility can hide valid mail handlers.
//
// Everything here is framework-free and takes canOpenURL/openURL/alert through
// dependency injection, so it can run under plain mocha without React Native.
// Settings and ExternalLink bind their platform-specific adapters.
//
 ...
 * platform-specific capability decision, the try/catch, and the fallback copy
 * exist once.
 *
 * Resolves in every path and never rejects — that is the whole point of the
 * change. The single try/catch spans the optional capability check and the
 * open, so a throw from either one lands on the same guarded exit.
 */
