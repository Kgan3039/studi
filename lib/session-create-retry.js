...
const SAFE_SESSION_MODERATION_ERROR = 'Please revise this text before posting it.';
...
function getCreateSessionErrorMessage(error) {
  if (error instanceof CreateSessionValidationError) return error.message;
  if (error && typeof error === 'object' && error.name === 'ObjectionableContentError') {
    return SAFE_SESSION_MODERATION_ERROR;
  }
  return SAFE_CREATE_SESSION_ERROR;
}
